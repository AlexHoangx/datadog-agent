...
EOF
  ...
  #Alex custom method
  if [ "$env" ]; then
      printf "\033[34m\n* Adding your ENVIRONMENT to the $nice_flavor configuration: $config_file\n\033[0m\n"
      $sudo_cmd sh -c "sed -i 's/# env: <environment name>/env: $env/' $config_file"
  fi
fi
